...
class NotMatchedClauseBuilder private[snowpark] (
   ...
   * }}}
   *
   * Adds a not matched clause where a row in source is not matched if its id does not equal
   * the id of any row in the [[Updatable]] target. For all such rows, insert a row
   * into target whose id and value are assigned to the id and value of the not matched row.
   *
   ...
   * Adds a not matched clause where a row in source is not matched if its id does not equal
   * the id of any row in the [[Updatable]] target. For all such rows, insert a row
   * into target whose id is assigned to the id of the not matched row.
   *
   * @group transform
   * @since 0.7.0
   * @return [[MergeBuilder]]
...
class MatchedClauseBuilder private[snowpark] (
  ...
  /**
   * Defines an update action for the matched clause, when a row in target is matched,
   * update the row in target with <assignments>, where the key specifies column name and value
   * specifies its assigned value. Returns an updated [[MergeBuilder]] with the new clause
   * added.
   *
   * For example:
   * {{{
   ...
   * }}}
   *
   * Adds a matched clause where a row in target is matched if its id equals the id of a
   * row in the [[DataFrame]] source. For all such rows, update its value to the value of the
  ...
  def update(assignments: Map[Column, Column]): MergeBuilder = {
    MergeBuilder(
      ...
      mergeBuilder.source,
      mergeBuilder.joinExpr,
      mergeBuilder.clauses :+ UpdateMergeExpression(condition.map(_.expr), assignments.map {
        case (k, v) => (k.expr, v.expr)
      }),
  ...
  /**
   * Defines a delete action for the matched clause, when a row in target is matched,
   * delete it from target. Returns an updated [[MergeBuilder]] with the new clause
   * added.
   *
   * For example:
   * {{{
   ...
   *
   * Adds a matched clause where a row in target is matched if its id equals the id of a
   * row in the [[DataFrame]] source. For all such rows, delete it from target.
   *
   * @group transform
   * @since 0.7.0
   * @return [[MergeBuilder]]
