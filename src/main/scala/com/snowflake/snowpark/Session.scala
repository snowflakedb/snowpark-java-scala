...
class Session private (private[snowpark] val conn: ServerConnection) extends Logging {
  ...
  /** Registers a file in stage or a local file as a dependency of a user-defined function (UDF).
    ...
    *
    * The following example demonstrates how to add dependencies on local files and files in a
    * stage:
    *
    * {{{
  ...
  /** Updates the query tag that is a JSON encoded string for the current session.
    ...
    *   If the provided query tag or the query tag of the current session are not valid JSON
    *   strings; or if it could not serialize the query tag into a JSON string.
    * @since 1.13.0
    */
  ...
  /** Returns a dataframe with only columns that are in the result of df.join but not the original
    * df
    *
    * @param df
