...
/** Represents a column or an expression in a DataFrame.
  ...
  *     [[com.snowflake.snowpark.DataFrame.apply(colName* DataFrame.apply]] method
  *     (`<dataframe>`("<col_name>")`).
  *
  * For example:
  *
...
case class Column private[snowpark] (private[snowpark] val expr: Expression) extends Logging {
  ...
  /** Returns a conditional expression that you can pass to the filter or where method to perform
    * the equivalent of a WHERE ... IN query with a specified list of values.
    *
    * The expression evaluates to true if the value in the column is one of the values in a
    * specified sequence.
    *
    * For example, the following code returns a DataFrame that contains the rows where the column
...
/** Represents a [[https://docs.snowflake.com/en/sql-reference/functions/case.html CASE]]
  * expression.
  *
