...
/** Provides methods to register a UDTF (user-defined table function) in the Snowflake database.
  ...
  * where ''n'' specifies the number of input arguments for your UDTF. For example, if your UDTF
  * passes in 3 input arguments, extend the `UDTF3` class.
  *
  * In your class, override the following three methods:
  *   - `process()` , which is called once for each row in the input partition.
  *   - `endPartition()`, which is called once for each partition after all rows have been passed to
  *     `process()`.
  *   - `outputSchema()`, which returns a [[types.StructType]] object that describes the schema for
  *     the returned rows.
  *
  ...
  *   - If the statement does not specify the PARTITION clause (implicit partitions), Snowflake
  ...
  *
  * The arguments passed to the registered UDTF are passed to `process()`. For each argument passed
  * to the UDTF, you must have a corresponding argument in the signature of the `process()` method.
  * Make sure that the type of the argument in the `process()` method matches the Snowflake data
  * type of the corresponding argument in the UDTF.
  *
  ...
  *
  * Construct and return a [[types.StructType]] object that uses an Array of [[types.StructField]]
  * objects to specify the Snowflake data type of each field in a returned row.
  *
  ...
  *
  * The arguments `start` and `count` specify the starting number for the row and the number of rows
  * to generate.
  *
  * {{{
  ...
  * [[UDTFRegistration]] methods. You can register a temporary or permanent UDTF by name. If you
  * don't need to call the UDTF by name, you can register an anonymous UDTF.
  *
  * ==Registering a Temporary UDTF By Name==
  *
  * To register a temporary UDTF by name, call `registerTemporary`, passing in a name for the UDTF
  * and an instance of the UDTF class. For example:
  * {{{
  ...
  *
  * When registering a permanent UDTF, you must specify a stage where the registration method will
  ...
  *
  * If you do not need to refer to a UDTF by name, use [[registerTemporary(udtf* UDTF)]] to create
  * an anonymous UDTF instead.
  *
  * ==Calling a UDTF==
...
class UDTFRegistration(session: Session) extends Logging {
  ...
  /** Registers an UDTF instance as a temporary Snowflake Java UDTF that you plan to use in the
    * current session.
    *
    * @param funcName
  ...
  /** Registers an UDTF instance as a Snowflake Java UDTF.
    *
    * The function uploads the JAR files that the UDTF depends upon to the specified stage. Each JAR
    * file is uploaded to a subdirectory named after the MD5 checksum for the file.
    *
    * If you register multiple UDTFs and specify the same stage location, any dependent JAR files
    ...
    *   The UDTF instance to be registered.
    * @param stageLocation
    *   Stage location where the JAR files for the UDTF and its and its dependencies should be
    *   uploaded
    * @since 1.2.0
    */
