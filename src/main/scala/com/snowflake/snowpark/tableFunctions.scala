...
object tableFunctions {
  ...
  /** Flattens (explodes) compound values into multiple rows.
    ...
    * Argument List:
    *
    * input: Required. The expression that will be unseated into rows. The expression must be of
    ...
    *
    * outer: Optional boolean value. If FALSE, any input rows that cannot be expanded, either
    ...
    *
    * recursive: Optional boolean value If FALSE, only the element referenced by PATH is expanded.
    * If TRUE, the expansion is performed for all sub-elements recursively. Default: FALSE
    *
    * mode: Optional String ("object", "array", or "both") Specifies whether only objects, arrays,
  ...
  /** Flattens (explodes) compound values into multiple rows.
    ...
    * @param outer
    *   Optional boolean value. If FALSE, any input rows that cannot be expanded, either because
    *   they cannot be accessed in the path or because they have zero fields or entries, are
    *   completely omitted from the output. If TRUE, exactly one row is generated for zero-row
