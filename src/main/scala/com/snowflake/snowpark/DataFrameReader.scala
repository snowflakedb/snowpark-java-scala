...
/** Provides methods to load data in various supported formats from a Snowflake stage to a
  * DataFrame. The paths provided to the DataFrameReader must refer to Snowflake stages.
  *
  * To use this object:
  *
  ...
  *      by calling the [[option]] or [[options]] method. These methods return a DataFrameReader
  *      that is configured with these options. (Note that although specifying copy options can make
  *      error handling more robust during the reading process, it may have an effect on
  *      performance.)
  ...
  *      [[json]], etc.). These methods return a [[DataFrame]] that is configured to load data in
