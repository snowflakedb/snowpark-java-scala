...
/** Represents a lazily-evaluated relational dataset that contains a collection of [[Row]] objects
  ...
  *
  *   - '''Transformations''' produce a new DataFrame from one or more existing DataFrames. Note
  *     that tranformations are lazy and don't cause the DataFrame to be evaluated. If the API does
  *     not provide a method to express the SQL that you want to use, you can use
...
class DataFrame private[snowpark] (
  ...
  def toDF(colNames: Seq[String]): DataFrame = transformation("toDF") {
    ...
    if (matched) {
      ...
      val newCols = output.zip(colNames).map { case (attr, name) =>
        Column(attr).as(name)
      }
      select(newCols)
    }
  ...
  private def renameBackIfDeduped(df: DataFrame): DataFrame = {
    ...
    if (!session.conn.hideInternalAlias || renamedColumns.isEmpty) {
      ...
      val filteredRenamedColumns = renamedColumns.filter { case (newName, _) =>
        resultColumnNames.contains(newName)
      }

      // columns has been de-duplicated
      val dedupColumns = filteredRenamedColumns
        .groupBy { case (_, oldName) =>
          oldName
        }
        .filter {
      ...
      val toBeRenamed = filteredRenamedColumns.filter { case (_, oldName) =>
        dedupColumns.contains(oldName)
      }

      val newRenamedMap = filteredRenamedColumns.filter { case (_, oldName) =>
        !dedupColumns.contains(oldName)
      }
  ...
  /** Performs an SQL
    ...
    * on the DataFrame.
    *
    * GROUP BY GROUPING SETS is an extension of the GROUP BY clause that allows computing multiple
    * GROUP BY clauses in a single statement. The group set is a set of dimension columns.
    *
    * GROUP BY GROUPING SETS is equivalent to the UNION of two or more GROUP BY operations in the
    * same result set:
    *
  ...
  /** Creates a new DataFrame by removing duplicated rows on given subset of columns. If no subset
    * of columns specified, this function is same as [[distinct()]] function. The result is
    * non-deterministic when removing duplicated rows from the subset of columns but not all
    * columns. For example: Supposes we have a DataFrame `df`, which contains three rows (a, b, c):
    * (1, 1, 1), (1, 1, 2), (1, 2, 3) The result of df.dropDuplicates("a", "b") can be either (1, 1,
    * 1), (1, 2, 3) or (1, 1, 2), (1, 2, 3)
    *
    * @group transform
    * @since 0.10.0
    * @return
  ...
  private def internalUnionByName(other: DataFrame, isAll: Boolean): DataFrame = {
    ...
    val matched: Boolean = if (leftOutputAttrs.size != rightOutputAttrs.size) {
      ...
      leftOutputAttrs.zip(rightOutputAttrs).forall { case (attribute, attribute1) =>
        attribute.name == attribute1.name
      }
    }
  ...
  /** Performs a default inner join of the current DataFrame and another DataFrame (`right`).
    ...
    * to one of these columns in the returned DataFrame, use the [[apply]] or [[col]] function on
    * the current or `right` DataFrame to disambiguate references to these columns.
    *
    * For example:
    *
    * {{{
  ...
  /** Joins the current DataFrame with the output of the specified table function `func`.
    ...
    * @since 0.4.0
    * @param func
    *   [[TableFunction]] object, which can be one of the values in the [[tableFunctions]] object or
  ...
  /** Joins the current DataFrame with the output of the specified table function `func` that takes
    ...
    *   an object that you create from the [[TableFunction]] class.
    * @param args
    *   Map of arguments to pass to the specified table function. Some functions, like `flatten`,
    *   have named parameters. Use this map to specify the parameter names and their corresponding
    *   values.
    */
  ...
  /** Joins the current DataFrame with the output of the specified table function `func`.
    ...
    * @param func
    *   [[TableFunction]] object, which can be one of the values in the [[tableFunctions]] object or
    *   an object that you create from the [[TableFunction.apply()]].
    */
  ...
  def withColumns(colNames: Seq[String], values: Seq[Column]): DataFrame =
    transformation("withColumns") {
      ...
      val newCols = qualifiedNames.zip(values).map { case (name, col) =>
        col.as(name).expr.asInstanceOf[NamedExpression]
      }
  ...
  private[snowpark] def showString(n: Int, maxWidth: Int = 50): String = {
    ...
    val header: Seq[String] = metaWithDisplayName.zipWithIndex.map { case (field, index) =>
    ...
    def splitLines(value: String): Seq[String] = {
      ...
      var startIndex = 0
      value.zipWithIndex.foreach { case (c, index) =>
        if (c == '\n') {
    ...
    val body: Seq[Seq[String]] = result.flatMap(row => {
      ...
      val lines: Seq[Seq[String]] = row.toSeq.zipWithIndex.map { case (value, index) =>
  ...
  /** Returns a new DataFrame that contains a sampling of rows from the current DataFrame.
    ...
    *     supports specifying 'probability' as a percentage number. The range of 'probability' is
    *     {@code [0.0, 100.0]} . The conversion formula is
  ...
  /** Randomly splits the current DataFrame into separate DataFrames, using the specified weights.
    ...
    *     DataFrame.
    *   - If multiple weights are specified, the current DataFrame will be cached before being
    *     split.
    *
    * @param weights
  ...
  /** Flattens (explodes) compound values into multiple rows (similar to the SQL
    ...
    *   OBJECT, or ARRAY.
    * @param path
    *   The path to the element within a VARIANT data structure which needs to be flattened. Can be
    *   a zero-length string (i.e. empty path) if the outermost element is to be flattened.
    * @param outer
    *   If FALSE, any input rows that cannot be expanded, either because they cannot be accessed in
    *   the path or because they have zero fields or entries, are completely omitted from the
    *   output. Otherwise, exactly one row is generated for zero-row expansions (with NULL in the
  ...
  /** Executes the query representing this DataFrame with statement parameters and returns the query
    * ID that represents its result. NOTE: The statement parameters are only used for the last
    * query.
    *
    * @param statementParameters
    *   The statement parameters map
    * @return
    *   the query ID
    */
  ...
  lazy private[snowpark] val methodChainString: String =
    methodChain.foldLeft("DataFrame") { case (str, methodName) =>
      s"$str.$methodName"
    }
