...
/** DataFrame for loading data from files in a stage to a table. Objects of this type are returned
  * by the [[DataFrameReader]] methods that load data from files (e.g. [[DataFrameReader.csv csv]]).
  *
...
class CopyableDataFrame private[snowpark] (
  ...
  /** Executes a `COPY INTO <table_name>` command with the specified transformations and options to
    ...
    *   [[https://docs.snowflake.com/en/sql-reference/sql/copy-into-table.html#transformation-parameters transformation parameters]]).
    * @param options
    *   Map of the names of options (e.g. {@code compression} , {@code skip_header} , etc.) and
    *   their corresponding values.NOTE: By default, the {@code CopyableDataFrame} object uses the
    *   options set in the [[DataFrameReader]] used to create that object. You can use this
  ...
  /** Executes a `COPY INTO <table_name>` command with the specified transformations and options to
    ...
    * in the `COPY INTO <table_name>` command.
    *
    * You can specify a subset of the table columns to copy into. The number of provided column
