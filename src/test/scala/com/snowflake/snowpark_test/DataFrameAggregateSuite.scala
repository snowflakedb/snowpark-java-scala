...
class DataFrameAggregateSuite extends TestData {
  ...
  test("SN - grouping and grouping_id") {
    ...
    /* TODO(SPARK_TEST_MOVE) with lazy analysis, exception is only thrown on collect
     * Add another test with eager analysis
     */
  ...
  test("SN - stddev") {
    ...
    checkAnswer(
      ...
      // math.sqrt(4 / 6.0) in Spark is 0.816496580927726, but it is 0.8164967850518458 in SF
  ...
  test("SN - moments") {
    ...
    checkAnswer(sparkVariance, Seq(Row(0.8)))
    ...
    while (varianceResult.next()) {
      checkAnswer(sparkVariance, Seq(Row(varianceResult.getDouble(1))))
    }
    statement.close()

    val sparkVariancePop = testData2.agg(var_pop($"a"))
    // 4.0 / 6.0 in Spark is 0.666666666666666, output in Snowflake is 0.666667
    checkAnswer(sparkVariancePop, Seq(Row(0.666667)))

    val sparkVarianceSamp = testData2.agg(var_samp($"a"))
    // 4.0 / 5.0 in Spark is 0.8, output in Snowflake is 0.80000
    checkAnswer(sparkVarianceSamp, Seq(Row(0.80000)))

    val sparkSkewness = testData2.agg(skew($"a"))
    /* TODO(SPARK_TEST_MOVE): Port this test method from spark tests
    ...
    checkAnswer(sparkKurtosis, Seq(Row(-1.875000000000)))
    ...
    while (aggKurtosisResult.next()) {
      checkAnswer(sparkKurtosis, Seq(Row(aggKurtosisResult.getDouble(1))))
    }
    statement.close()
  }
  ...
  test("SN - SPARK-14664: Decimal sum/avg over window should work.") {
    checkAnswer(
  ...
  test("SN - SPARK-21580 ints in aggregation expressions are taken as group-by ordinal.") {
    checkAnswer(
